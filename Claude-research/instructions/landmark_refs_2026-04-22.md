...
**Date:** 2026-04-22
**Schema version:** 2026-04-22
**Status:** Authoritative. Derived from
`.status/landmark_refs_proposed_2026-04-22.md` after user sign-off on
2026-04-22. That file is the review record (substitutions, second-pass
log, user decisions); this file is the clean list for downstream use.

This document is the source of truth for the landmark override list used
by the HED literature-search workstream. Phase 2 triage will use these
entries as unconditional keeps regardless of venue score or citation
count. DOI resolution is handled by a Phase 1 batch script (CrossRef
lookup via `outputs/literature_search/clients/crossref.py`) that
serializes this content to `outputs/literature_search/landmark_refs.json`.

## Criteria

An entry earns landmark status if it is one of:

1. The **origin paper** that named the construct or introduced the task.
2. The **canonical modern review** — the integrative review textbooks cite.
3. The **method-defining paper** that introduced the analytical or
   computational framework now inseparable from the construct.

Excluded by rule: test manuals (WAIS, WMS, IAPS normative ratings),
citation-count-based inclusion, general textbooks, and book chapters.
One book-chapter exception is carved out — see the appendix.

Confidence is flagged per entry: **[H]** high (well-known canonical),
**[M]** medium (good candidate but the field has alternatives), **[L]**
low (best guess; please verify or replace).

## 1. Process landmarks

### 1.1 Associative Learning and Reinforcement

| process_id | Citation | Title and venue | Conf. |
|---|---|---|---|
...
### 1.2 Auditory and Pre-Attentive Deviance Processing
...
### 1.3 Awareness, Agency, and Metacognition
...
| hed_self_monitoring | Nelson & Narens (1990) [book_chapter_exception] | "Metamemory: a theoretical framework and new findings." *Psychol Learn Motiv* 26:125–173 | H |
...
### 1.4 Cognitive Control Strategy and Flexibility
...
### 1.6 Face, Object, and Scene Perception
...
### 1.7 Implicit Learning and Procedural Memory
...
### 1.8 Executive Control and Inhibition
...
### 1.10 Long-Term Memory: Encoding, Consolidation, and Retrieval
...
### 1.14 Reward, Motivation, and Effort
...
### 1.15 Selective and Sustained Attention
...
### 1.16 Short-Term and Working Memory
...
### 1.19 Value-based Decision Making, Risk, and Reward Processing
...
Organized alphabetically by task_id.

| task_id | Citation | Title and venue | Conf. |
|---|---|---|---|
...
## Appendix A. Processes without a landmark entry

About 88 processes are deliberately left without a landmark. These are
processes where no single paper stands out as the canonical origin or
review — either because the process is too umbrella (e.g., most
perception submodalities, umbrella motor processes, umbrella
decision/social/reasoning constructs) or because the canonical reference
is in an excluded venue class (WAIS-based tasks cited to the test manual,
approach/avoidance motivation cited to Gray's textbook). Triage's
venue-allowlist and keep/drop rules handle these without a landmark
override. The full list is recorded in §4 of the proposal file.

## Appendix B. Carved-out exceptions

The no-book-chapter rule has one formal exception:

- `hed_self_monitoring` → **Nelson & Narens (1990)** "Metamemory: a
  theoretical framework and new findings" *Psychol Learn Motiv*
  26:125–173. The object-level / meta-level metamemory framework is the
  theoretical foundation of the performance-monitoring literature; the
  book-chapter venue is incidental. Flagged in the JSON with
  `book_chapter_exception: true` for traceability. User decision,
  2026-04-22.

## Appendix C. Downstream JSON schema

After DOI resolution, this list is serialized to
`outputs/literature_search/landmark_refs.json`:
  ...
  "entries": [
    {
      ...
      "venue": "Percept Psychophys 16:143–149",
      "doi": "10.3758/BF03203267",
      "confidence": "H",
      "book_chapter_exception": false
    }
  ]
}
```

DOI resolution uses CrossRef author+year+title lookup via the Phase 1
client at `outputs/literature_search/clients/crossref.py`. Each resolved
entry is also verified against OpenAlex for cross-source agreement
before writing to the JSON.
