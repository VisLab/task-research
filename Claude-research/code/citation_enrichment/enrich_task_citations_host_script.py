...
_P_REPORT = re.compile(
    ...
    r"unpublished|dissertation|thesis|working paper)\b",
    re.IGNORECASE,
)
_P_PUBLISHER = re.compile(
    ...
    r"guilford|erlbaum|routledge|taylor|mcgraw|pearson|sage|apa|aps)\b",
    re.IGNORECASE,
)
...
def _stable_key(cs, journal, year):
    return json.dumps([cs, journal, year], sort_keys=True)
...
def _fmt_oa_authors(authorships):
    parts = [((a.get("author") or {}).get("display_name") or "").strip() for a in authorships]
...
def _try_crossref(cs, year, session):
    ...
    for item in items[:3]:
        ...
        return ResolvedReference(
            authors=_fmt_cr_authors(cr_authors),
            year=iy,
...
def _try_openalex(cs, year, session):
    ...
    for item in results[:3]:
        ...
        return ResolvedReference(
            authors=_fmt_oa_authors(authorships),
            year=iy,
...
def _try_europepmc(cs, year, journal, session):
    ...
    for item in results[:3]:
        ...
        if api_authors and not _author_match(cs, api_authors):
            continue
        authors_str = (
            "; ".join(f"{a.get('lastName', '')}, {a.get('initials', '')}".strip(", ") for a in al if a.get("lastName"))
            or None
        )
...
def _try_semanticscholar(cs, year, session):
    stripped = re.sub(r"\s+", " ", _strip_md(cs)).strip()
    params = {"query": stripped, "limit": "3", "fields": "externalIds,title,authors,venue,year"}
    if year:
        params["year"] = str(year)
    resp = _get("https://api.semanticscholar.org/graph/v1/paper/search", params, "api.semanticscholar.org", session)
    if not resp:
        return None
    try:
...
def resolve_reference(citation_string, venue, year, cache_dir, session):
    ...
    if result is None:
        result = _try_europepmc(citation_string, year, venue, session)
    if result is None:
...
def main():
    ...
    ap.add_argument(
        "--workspace", default=None, help="Path to workspace root (default: parent of this script's directory)"
    )
    ap.add_argument("--dry-run", action="store_true", help="Parse only; no network calls, no file writes.")
    ap.add_argument(
        "--write-back", action="store_true", help="Write enriched data back to task_details.json (with backup)."
    )
    args = ap.parse_args()
    ...
    for ti, (orig, enr) in enumerate(zip(tasks, enriched_tasks, strict=False)):
    ...
    n_resolved = sum(
        1 for (ti, rk, ri, s) in all_refs if resolved_map[(ti, rk, ri)].source not in ("unresolved", "historical")
    )
    n_with_doi = sum(1 for (ti, rk, ri, s) in all_refs if resolved_map[(ti, rk, ri)].doi)
    print()
    print("=" * 50)
