...
def _parse_for_fallback(cs):
    ...
    if not year_m:
        return {"authors": "", "year": None, "title": cs[:120], "venue": None, "venue_type": "other", "doi": doi}
    ...
    return {"authors": authors, "year": year, "title": title, "venue": venue, "venue_type": venue_type, "doi": doi}
...
for task in tasks:
    for rk in ("key_references", "recent_references"):
        for ref in task.get(rk, []):
            ...
            if (
                src not in ("unresolved", "historical", "needs_review")
                and cs_au
                and res_au
